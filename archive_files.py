...
parser = argparse.ArgumentParser(
    description='Archive and move files (using gzip).')
...
parser.add_argument(
    '-r', '--regex', help='Regular expression.  Default = ".*"', default='.*')
            ...
            if os.path.isfile(full_path):
                ...
                if file_regex.search(f):
                    ...
                    if os.stat(full_path).st_mtime < now - (days_old * 86400):
                        ...
                        destination_archived_file = os.path.join(
                            full_path_source_archived_file.split('/')[-1])
                        full_path_destination_archived_file = os.path.join(
                            args.archive, destination_archived_file)
                        try:
                            shutil.move(full_path_source_archived_file,
                                        full_path_destination_archived_file)
                        except IOError as err:
                            print('Error: ', err)
                            pass
    else:
